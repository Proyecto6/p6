...
<body id="page-top">
  ...
  <div class="header22">
    <br/><br/><br/>
              ...
              <form Name="preguntastribuna" action="notaalumno.proc.php" method="post">
      <?php
      $alunombre = array();

      $aluid = array();
      //primero realizamos una consulta para obtener las matriculas de los integrantes del proyecto seleccionado.
      $consulta= "SELECT usa_id FROM  tbl_participantes WHERE pro_id =".
    ...
    //y ahora para cada nombre de la array hacemos una consulta para generar las preguntas a cada alumno
        foreach ($alunombre as $nom_alu) {
          $sql= "SELECT * FROM tbl_preguntasalumno WHERE pa_tipo = 0";
          //echo $sql;
          $preguntas_alumno= mysqli_query($conexion, $sql) or die (mysqli_error());
            ...
            <input style='color:black;' type='number' name='notas[]' min='0' max='10' value='5'> <br><br>";
            echo"<input type='hidden' name='idpreguntas[]' value='".$pregunta['pa_id']."' >";
      ...
      //y ahora generamos las preguntas globales de la presentación
        ...
        $sql= "SELECT * FROM tbl_preguntasalumno WHERE pa_tipo = 1";
        ...
        while($pregunta = mysqli_fetch_array($preguntas_alumno)){
          ...
          <input type='number' name='notas_globales[]' min='0' max='10' value='5'> <br><br>";

          echo"<input type='hidden' value=".$pregunta['pa_id']." name='pa_id[]' >";
